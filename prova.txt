...
if utente_connesso:
    ...
    # --- TAB 1: ACQUISIZIONE E ANALISI (NUOVI REPORT) ---
    with tab1:
            ...
            if st.session_state.get("last_audio_hash") != audio_hash:
                with st.spinner("Analisi in corso..."):
                    ...
                    st.session_state.storico_report.append({
                        "nome_file": file.name, "report": report, "trascrizione": testo, 
                        "bytes": file.getvalue(), "version": 1
                    })
        ...
        if st.session_state.storico_report:
            for idx, data in enumerate(st.session_state.storico_report):
                ...
                with st.expander(f"🔍 {titolo.upper()} ({nome_file})", expanded=True):
                    ...
                    with col1:
                        st.image(disegna_punti_critici(data["bytes"], punti_totali, abilita_marker=mostra_marker), use_container_width=True)
