...
if utente_connesso:
    ...
    with tab1:
        ...
        # Avviso nella Tab 1
        if "in_rework_mode" in st.session_state:
            idx = st.session_state.in_rework_mode
            st.warning(f"⚠️ **RE-REGISTRAZIONE:** {st.session_state.storico_report[idx]['nome_file']}")
        
        if "in_integration_mode" in st.session_state:
            idx = st.session_state.in_integration_mode
            st.info(f"🎤 **INTEGRAZIONE:** Aggiungi dettagli a {st.session_state.storico_report[idx]['nome_file']}")
        
        st.subheader("📸 Carica un'immagine")
        
        # Gestiamo il file uploader con una key specifica per rilevare il cambio
        file = st.file_uploader("Carica una foto", type=["jpg", "jpeg", "png"], key="uploader_cantiere")
        
        # Logica di reset: se il file caricato cambia, resettiamo l'audio registrato
        if "last_file_name" not in st.session_state:
            st.session_state.last_file_name = None
            
        if file is not None and file.name != st.session_state.last_file_name:
            st.session_state.last_file_name = file.name
            # Resettiamo l'audio in sessione quando cambia il file
            if "recorder" in st.session_state:
                del st.session_state["recorder"]
            st.rerun() # Ricarichiamo per pulire il componente mic_recorder

        # Ora il mic_recorder partirà sempre "pulito" finché non premi il tasto
        audio = mic_recorder(start_prompt="⏺️ AVVIA REGISTRAZIONE", stop_prompt="⏹️ FERMA E ANALIZZA", key='recorder')
        ...
        if audio and file:
            ...
            if st.session_state.get("last_audio_hash") != audio_hash:
                with st.spinner("Analisi in corso..."):
                    ...
                    # 1. Decidiamo che tipo di analisi eseguire
                    idx = st.session_state.get("in_rework_mode")
                    integ_idx = st.session_state.get("in_integration_mode")
                    
                    if idx is not None:
                        # MODALITÀ REWORK (Sostituzione totale)
                        report, testo = analizza_sicurezza_cantiere(audio['bytes'], file)

                    elif integ_idx is not None:
                        # MODALITÀ INTEGRAZIONE (Merge con verbale esistente)
                        idx = integ_idx
                        verbale_attuale = st.session_state.edits.get(f"edit_testo_{idx}", "")
                        report, testo = integra_sicurezza_cantiere(audio['bytes'], file, verbale_attuale)

                    else:
                        # MODALITÀ NORMALE (Nuovo report)
                        report, testo = analizza_sicurezza_cantiere(audio['bytes'], file)
                    
                    # 2. Aggiornamento dello Stato
                    new_version = st.session_state.get("version_counter", 0) + 1
                    st.session_state.version_counter = new_version
                    
                    # Logica di salvataggio
                    if idx is not None:
                        # Aggiornamento record esistente (REWORK o INTEGRAZIONE)
                        st.session_state.storico_report[idx] = {
                            "nome_file": file.name, 
                            "report": report, 
                            "trascrizione": testo, 
                            "bytes": file.getvalue(),
                        ...
                        # Pulizia vecchi editi per forzare il refresh della UI
                        keys_da_cancellare = [k for k in st.session_state.edits.keys() if f"_{idx}" in k]
                        for k in keys_da_cancellare:
                            del st.session_state.edits[k]
                        
                        # Inseriamo il nuovo testo per la UI
                        st.session_state.edits[f"edit_testo_{idx}"] = testo
                        
                        # Pulizia flag
                        if "in_rework_mode" in st.session_state: del st.session_state["in_rework_mode"]
                        if "in_integration_mode" in st.session_state: del st.session_state["in_integration_mode"]
                        st.success("✅ Analisi aggiornata!")
        ...
        if st.session_state.storico_report:
            for idx, data in enumerate(st.session_state.storico_report):
                ...
                with st.expander(f"🔍 {titolo.upper()} ({nome_file})", expanded=True):
                    ...
                    with col1:
                        st.image(disegna_punti_critici(
                            data["bytes"], 
                            punti_totali, 
                            abilita_marker=mostra_marker
                        ...
                        if c1.button("🗑️ Elimina", key=f"del_{idx}"):
                            ...
                            # Pulizia anche degli edit associati per evitare orfani
                            keys_da_cancellare = [k for k in st.session_state.edits.keys() if f"_{idx}" in k]
                            for k in keys_da_cancellare: del st.session_state.edits[k]
                            st.rerun()
                            
                        if c2.button("🔄 Rifai", key=f"redo_{idx}"):
                            st.session_state.in_rework_mode = idx
                            st.rerun()
                            
                        if c3.button("➕ Integra", key=f"int_{idx}"):
                            st.session_state.in_integration_mode = idx
                            st.rerun()
